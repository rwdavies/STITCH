...
Rcpp::NumericVector rcpp_calculate_hwe_p(const Rcpp::NumericVector reference_hap) {
  ...
  Rcpp::NumericVector x = reference_hap;
  if (x[2] > x[0]) {
    double a = x[2];
    x[2] = x[0];
    x[0] = a;
  ...
  long int nAA = x[0]; // individuals
  long int nAB = x[1]; // individuals
  long int nBB = x[2]; // individuals
  long int nA = nAA * 2 + nAB; // number of copies of A allele
  long int nB = nBB * 2 + nAB; // number of copies of B allele
  long int n = nAA + nAB + nBB; // number of (diploid) individuals

  long int min_het = 0;
  long int max_het = nAB + 2 * std::min(nAA, nBB);
  ...
  long int n_het = mid;
  long int n_hom_alt = (nBB * 2 + nAB - n_het) / 2;
  long int n_hom_ref = (n - n_het - n_hom_alt);
  if ((mid - 2) >= min_het) {
  ...
  if ((mid + 2) <= max_het) {
      for(long int het = mid + 2; het <= max_het; het+=2) {            
          probs[het] = probs[het - 2] * \
