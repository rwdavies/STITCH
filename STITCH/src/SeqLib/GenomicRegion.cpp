...
namespace SeqLib
{
    ...
    void GenomicRegion::Pad(int32_t pad)
    {
        ...
        if (-pad * 2 > Width())
            throw std::out_of_range("GenomicRegion::pad - negative pad values can't obliterate GenomicRegion with val " + tostring(chr) + ":" + tostring(pos1) +
                                    "-" + tostring(pos2) + " and pad " + tostring(pad));
    ...
    {
        return ChrName(h) + ":" + SeqLib::AddCommas<int>(pos1) + "-" + AddCommas<int>(pos2) + "(" + strand + ")";
    }


    std::ostream& operator<<(std::ostream& out, const GenomicRegion& gr)
    {
        out << gr.chrToString(gr.chr) << ":" << SeqLib::AddCommas<int>(gr.pos1) << "-" << AddCommas<int>(gr.pos2) << "(" << gr.strand << ")";
        return out;
    }
