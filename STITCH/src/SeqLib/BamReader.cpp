    ...
    */
    ...
    bool BamReader::Open(const std::string& bam)
    {
        ...
        if (!m_cram_reference.empty())
            new_bam.m_cram_reference = m_cram_reference;
    ...
    bool BamReader::GetNextRecord(BamRecord& r)
    {
        ...
        for (_BamMap::iterator bam = m_bams.begin(); bam != m_bams.end(); ++bam)
        {
            ...
            { // error sent back from sam_read1
                // run time error
                std::stringstream ss;
    ...
    int32_t _Bam::load_read(BamRecord& r)
    {
        ...
        if (valid < 0)
        { // read still not found
            do
            {

#ifdef DEBUG_WALKER
                std::cerr << "Failed read, trying next region. Moving counter to " << m_region_idx << " of " << m_region.size() << " FP: " << fp_htsfile
                          << " hts_itr " << std::endl;
#endif
