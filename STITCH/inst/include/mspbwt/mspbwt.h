...
class msPBWT
{
  ...
  public:
    ...
    void build(const std::string & vcfpanel,
               ...
               double maf = 0)
    {
        ...
        for(m = 0; m < M; m++)
        {
            ...
            if((m + 1) % B == 0)
            {
                for(i = 0; i < N; i++) X[k][i] = reverseBits(X[k][i]); // reverset bits
                k++; // update next grid
            }
        }
        if(G == k + 1)
    ...
    void query(const std::string & vcfquery,
               const std::string & samples,
               const std::string & region,
               int viewk = 0)
    {
                         ...
                         int viewk = -1)
    {
        ...
        for(k = 0; k < gv.size(); k++)
        {
            ...
            {
                ghost.push_back(k);
                if(verbose) cerr << "ghost symbol at k: " << k << endl;
            ...
            else if(k > valid_grid_start)
            {
                ...
                if(f1 == g1)
                {
                    ...
                    for(i = f; i < g; i++)
                    {
                        ...
                        {
                            j = 0, klen = 0;
                            while(k >= ++j && X[gv[k - j]][n] == zg[gv[k - j]]) klen++;
                        }
                        // check if all equals
                        if(debug)
                        {
    ...
    void impute_setmaximal(int iind,
                           IntMapU & haplens,
                           IntMapU & hapends,
                           IntMapU & hapnindicies,
                           GridVec & zg,
    ...
    void report_setmaximal(IntMapU & haplens,
                           IntMapU & hapends,
                           IntMapU & hapnindicies,
                           GridVec & zg,
                           int viewk = -1)
    ...
    void report_neighourings(IntMapU & haplens,
                             ...
                             int L = 32)
    {
        int k, s, klen, j, n, l, Gi, ki, iind, ni, maxL;
        for(ni = -1, iind = 0; iind < nindices; iind++)
        {
            auto Gv = seq_by(iind, G - 1, nindices);
            Gi = Gv.size();
            ...
            for(ki = 0; ki < Gi; ki++)
            {
                ...
                maxL = W[ni][s].size();
                // zak_curr = nstart > 0 ? C[ni][s] + maxL - nstart : C[ni][s];
                zak_curr = C[ni][s];
