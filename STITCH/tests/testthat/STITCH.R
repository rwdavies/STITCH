...
option_list <- list(
    ...
    make_option(
        "--tempdir",
        help = "What directory to use as temporary directory. If possible, use ramdisk, like /dev/shm/"
    ), 
    make_option(
        "--bamlist",
...
STITCH(
    ...
    shuffleHaplotypeIterations = opt$shuffleHaplotypeIterations,
    splitReadIterations = opt$splitReadIterations,
    nCores = opt$nCores,
    expRate = opt$expRate,
    maxRate = opt$maxRate,
    minRate = opt$minRate,
    ...
    refillIterations = opt$refillIterations,
