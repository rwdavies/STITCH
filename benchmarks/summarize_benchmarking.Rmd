...
profile_whole_chr <- function(small = TRUE) {
	...
	mega_ctime <- file.info(paste0(STITCH_home, "benchmark-results/whole_chr_CFW_", name, ".megamuga.txt"))$ctime
        ...
        return(c(version, option, round(time_min, 1), round(acc_mega, 3), round(acc_affy, 3), as.character(mega_ctime)))
    }))
    colnames(whole_chr) <- c("Version", "Options", "Time (min)", "Accuracy (Mega)", "accuracy (Affy)", "Mega ctime")
    if (small) {
...
profile_smaller_chunk <- function(K, input, small = TRUE) {
    profile <- t(sapply(1:n, function(i) {
	    ...
	    log_ctime <- file.info(log)$ctime
            ...
            if (version == "1.1.1" & input == "CRAMS")
                time_min <- NA
            ## change name
            return(c(version, option, input, K, round(time_min, 1), as.character(log_ctime)))
    }))
    colnames(profile) <- c("Version", "Options", "Input", "K", "Time (min)", "Log ctime")
    if (small) {
       profile <- profile[, 1:5]
    }  
    return(profile)
